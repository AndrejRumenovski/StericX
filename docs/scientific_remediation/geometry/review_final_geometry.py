...
def compare_lane(name, before, after):
    ...
    for old, new, old_request, request in itertools.zip_longest(
        ...
        if "topology" in name:
            ...
            # The additional topology lane explicitly spells out these observer
            # defaults. Their absence historically meant false in the adapter.
            for flag in ["dump", "include_points"]:
                if flag not in old_request and projected.get(flag) is False:
                    projected.pop(flag)

...
def reduce_counts(frames,p):
    ...
    def total(xs):
        ...
        return t
    def python_sum(xs):
        # CPython 3.12.13 builtin_sum_impl, finite exact-float branch.
        t=Q(0);c=Q(0)
        for x in xs:
            new=rnd(t+x)
            correction=rnd(rnd(t-new)+x) if abs(t)>=abs(x) else rnd(rnd(x-new)+t)
            c=rnd(c+correction);t=new
        return rnd(t+c) if c else t
    regional_sum=total if p==24 else python_sum
    ...
    for f in frames:
        ...
        out.append(dict(buried_volume=rnd(frac(sum(ns),sum(ds))*volume),near_vbur=regional_sum(octs[4:]),far_vbur=regional_sum(octs[:4]),octants=octs,quadrants=qs))

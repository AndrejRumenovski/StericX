//! Checked-API remediation measurement adapter. NOT an independent reference.
use glam::Vec3;
...
#[allow(dead_code)]
mod observed_bv {
    include!("observed_buried_volume.rs");
    include!("topology_append.rs");
}
...
fn geometry(v:&Value, explicit_topology:bool)->Value {
    ...
    out["sterimol_bond"]=match SterimolCalculator::compute(&m,idx(v,"attach",d),idx(v,"sterimol_neighbor",r)){Ok(p)=>sterimol(p),Err(e)=>json!({"error":e.to_string()})};
    ...
    let n=if let Some(a)=v["neighbors"].as_array(){if a.len()==3{Some([a[0].as_u64().unwrap() as usize,a[1].as_u64().unwrap() as usize,a[2].as_u64().unwrap() as usize])}else{None}}else if !explicit_topology && neighbors.len()==3 {Some([neighbors[0].1,neighbors[1].1,neighbors[2].1])}else{None};
    out["pyramidalization"]=match n {Some(n)=>{match PyramidalizationCalculator::compute(&m,d,n){Ok(p)=>floats(&[("pyr_p",p.pyr_p),("pyr_alpha",p.pyr_alpha)]),Err(e)=>json!({"error":e.to_string()})}},None=>json!({"error":"observer requires exactly three explicit or inferred neighbors"})};
    let center=if let Some(center)=v.get("center"){Ok(vec3(center))}else if explicit_topology { n.ok_or_else(|| "explicit topology requires three neighbors".to_owned()).and_then(|n|steric_x::geometry::coordination_center_with_neighbors(&m,d,n,c).map_err(|e|e.to_string())) } else{steric_x::coordination_center(&m,d,r,c).map_err(|e|e.to_string())};
    match center {Ok(center)=>{out["center"]=json!(center.to_array());match SterimolCalculator::compute_with_dummy(&m,d,center){Ok(p)=>{out["sterimol_dummy_raw"]=sterimol(p);out["sterimol_coordination"]=sterimol(SterimolParams{l:p.l+0.40,..p});},Err(e)=>{out["sterimol_dummy_raw"]=json!({"error":e.to_string()});out["sterimol_coordination"]=json!({"error":e.to_string()});}}},Err(ref e)=>{out["center"]=json!({"error":e});out["sterimol_dummy_raw"]=json!({"error":e});out["sterimol_coordination"]=json!({"error":e});}}
    if !v["sterimol_only"].as_bool().unwrap_or(false) {
        let result=if explicit_topology {n.ok_or_else(|| "explicit topology requires three neighbors".to_owned()).and_then(|n| {
            if let Some(x)=v.get("center"){BV::compute_with_center_and_neighbors(&m,d,n,vec3(x),c)}else{BV::compute_with_neighbors(&m,d,n,c)}.map_err(|e|e.to_string())
        })}else{if let Some(x)=v.get("center"){BV::compute_with_center(&m,d,r,vec3(x),c)}else{BV::compute(&m,d,r,c)}.map_err(|e|e.to_string())};
        out["buried_volume"]=match result{Ok(p)=>bv(p),Err(e)=>json!({"error":e.to_string()})};
        if v["dump"].as_bool().unwrap_or(false){out["dump"]=if explicit_topology {match n {Some(n)=>observed_bv::audit_dump_with_neighbors(&m,d,n,v.get("center").map(vec3),copied_config(c),v["include_points"].as_bool().unwrap_or(false)),None=>json!({"error":"explicit topology requires three neighbors"})}}else{observed_bv::audit_dump(&m,d,r,v.get("center").map(vec3),copied_config(c),v["include_points"].as_bool().unwrap_or(false))};}
    }
    out
}
...
fn observe(v:&Value)->Value {match v["op"].as_str().unwrap_or("geometry") {"geometry"=>geometry(v,false),"geometry_topology"=>geometry(v,true),"kinetics"=>kinetics(v),"aggregate"=>aggregate(v),"model"=>model_observation::observe(v),"raw_occupancy"=>{let points=v["points"].as_array().unwrap().iter().map(vec3).collect::<Vec<_>>();let atoms=v["atoms"].as_array().unwrap().iter().map(|a|(vec3(&a["position"]),num(&a["radius_squared"]))).collect::<Vec<_>>();observed_bv::audit_raw(&points,&atoms,field(v,"sphere_radius",3.5))},_=>json!({"error":"unknown operation"})}}
fn main() {

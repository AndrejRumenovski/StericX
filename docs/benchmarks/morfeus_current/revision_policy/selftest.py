...
# Independent synthetic count vectors exercise Python's compensated region sum.
for _ in range(200):
 frames=[dict(occupied_counts=[rng.randrange(1927) for _ in range(8)],grid_populations=[1926]*8) for _ in range(3)]
 modeled,values,volume=r.reduce_counts(frames,53)
 for row in modeled:
  assert row['near_vbur']==sum(row['octants'][4:])
  assert row['far_vbur']==sum(row['octants'][:4])
 for key in ['buried_volume','near_vbur','far_vbur']:
  assert values[key]==float(np.mean([x[key] for x in modeled]))
print(json.dumps({'compensated_sum_synthetic_cases':600,'passed':True,'candidate_results_read':False}))
